...
export default function App() {
  ...
  useEffect(() => {
    const initializeFirebase = async () => {
      ...
      try {
        ...
        if (success) {
          console.log('✅ Firebase Auth + Cloudinary initialized');
          ...
          if (healthCheck.connected) {
            ...
            console.log('   Firebase Auth + Cloudinary integration active');
            if (healthCheck.cloudinaryAvailable) {
              console.log('☁️ Cloudinary is active - data will sync to cloud!');
            } else {
  ...
  const handlePickImage = async () => {
    ...
    if (!result.canceled) {
      ...
      if (base64) {
        // Convert to base64 data URI (persists in AsyncStorage and Cloudinary)
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      console.log('💾 Updating user data in Firebase Auth + Cloudinary...');
      const updateData = {
      ...
      if (updateResult.success) {
        ...
        // Reload user data from Cloudinary to get latest
        console.log('🔄 Refreshing user data from Cloudinary...');
        const freshUserData = await authService.getCurrentUser();
        
        if (freshUserData.success && freshUserData.user) {
          console.log('✅ Fresh data loaded from Cloudinary');
          console.log('📸 VERIFICATION - Photo in Cloudinary:');
          ...
          if (freshUserData.user.profilePhoto) {
            ...
            console.error('❌ PHOTO NOT SAVED TO CLOUDINARY!');
    ...
    <View style={styles.screen}>
      ...
      >
        ...
        <Text style={styles.firebaseStatusText}>
          {firebaseStatus === 'connected' ? 'AGROF: Firebase Auth + Cloudinary' : 
  ...
  const renderAccountScreen = () => (
    <View style={styles.screen}>
      ...
      <ScrollView style={styles.tabContent} showsVerticalScrollIndicator={false}>
        ...
        {/* Seller Section */}
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Become a Seller</Text>
          
          <View style={styles.sellerCard}>
            <MaterialIcons name="store" size={40} color="#4CAF50" />
            <Text style={styles.sellerTitle}>Sell on AGROF</Text>
            <Text style={styles.sellerDescription}>
              Start selling your agricultural products and reach thousands of farmers across Uganda!
            </Text>
            ...
            <TouchableOpacity 
              ...
              onPress={() => {
                Alert.alert(
                  '📧 Contact Us to Sell on AGROF',
                  'Email: support@agrof.farm\nPhone: +256 705 223 777\n\nWe\'ll help you get started!',
                  [
                    { text: 'Call', onPress: () => Linking.openURL('tel:+256705223777') },
                    { text: 'Email', onPress: () => Linking.openURL('mailto:support@agrof.farm') },
            ...
            >
              ...
              <Text style={styles.sellerButtonText}>Contact Us</Text>
            </TouchableOpacity>
  ...
  const getBackgroundImage = () => {
    ...
    // Home tab - smart farming dashboard
    if (currentTab === 'home') {
      console.log('🎨 Using welcome background for home tab');
      return 'welcome';     // Home tab - welcome.png background
    }
