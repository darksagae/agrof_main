...
const ProductRecommendationCards = ({ diseaseType, symptoms, cropType, onProductPress }) => {
  ...
  const fetchBasicRecommendations = async () => {
    try {
      ...
      // Get fresh product data using hybrid API (works offline!)
      let allProductsResponse = [];
      try {
        console.log('🌐 Fetching product data with hybrid API (offline-ready)...');
        const result = await hybridStoreApi.getProducts({ limit: 500 });
        allProductsResponse = result.data;
        console.log(`✅ Products loaded from ${result.source}: ${allProductsResponse.length} items`);
        console.log(`💾 Data used: ${result.dataUsed || 0} KB`);
        console.log(`📦 Found ${allProductsResponse.length} total products`);
        ...
        console.error('❌ Product fetch failed:', apiError);
        // Hybrid API should handle this, but if it fails completely:
        allProductsResponse = [];
        console.warn('⚠️ No products available - hybrid API failed');
      }
      ...
      // Last resort: try to get any products using hybrid API (works offline!)
      try {
        console.log('🆘 Last resort: fetching products with hybrid API...');
        const result = await hybridStoreApi.getProducts({ limit: 6 });
        console.log(`🆘 Emergency products from ${result.source}: ${result.data.length}`);
        setRecommendedProducts(result.data.slice(0, 6));
  ...
  const fetchCropSpecificProducts = async (cropType) => {
    try {
      ...
      // Fetch product data using hybrid API (works offline!)
      console.log('🔄 Fetching product data with hybrid API...');
      const result = await hybridStoreApi.getProducts({ limit: 100 });
      const allProducts = result.data;
      console.log(`📦 Products loaded from ${result.source}: ${allProducts.length}`);
