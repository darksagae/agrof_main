...
const StoreScreen = () => {
  ...
  useEffect(() => {
    // Fast initial load with timeout
    const loadData = async () => {
      try {
        ...
        console.warn('⚠️ Load timeout, using fallback data');
        setLoading(false);
    ...
    const startMarquee = () => {
      ...
      const animate = () => {
        Animated.timing(marqueeAnimation, {
          toValue: 1,
          duration: 90000, // 90 seconds for extremely slow, very comfortable reading
  ...
  const loadStoreData = async () => {
    try {
      ...
      // Fast loading - skip network manager initialization
      console.log('📡 Using fast loading mode...');
      
      // Use hybrid API that works offline - with timeout for faster loading
      const [health, categoriesResult, productsResult] = await Promise.allSettled([
        Promise.race([
          healthCheck(),
        ...
        Promise.race([
          ...
          console.warn('⚠️ Categories error:', err);
          return { source: 'offline', data: [] };
        }),
        Promise.race([
          hybridStoreApi.getProducts({ limit: 100 }), // Reduced limit for faster loading
          new Promise((_, reject) => setTimeout(() => reject(new Error('Products timeout')), 3000))
        ]).catch(err => {
          console.warn('⚠️ Products error:', err);
          return { source: 'offline', data: [] };
        })
      ]);
      
      // Extract data from hybrid API responses
      ...
      if (health.status === 'fulfilled' && health.value) {
        ...
        console.warn('⚠️ Health check failed, using OFFLINE mode');
        setBackendStatus('offline');
      }
      
      // Handle categories result from hybrid API
      if (categoriesData && categoriesData.data) {
        console.log(`📂 Categories loaded from ${categoriesData.source}:`, categoriesData.data.length, 'categories');
        setCategories(categoriesData.data.length > 0 ? categoriesData.data : fallbackCategories);
      } else {
        console.warn('⚠️ Categories loading failed, using fallback');
        setCategories(fallbackCategories);
      }
      
      // Handle products result from hybrid API
      if (productsData && productsData.data) {
        console.log(`⭐ Products loaded from ${productsData.source}:`, productsData.data.length, 'products');
        console.log(`💾 Data used: ${productsData.dataUsed || 0} KB`);
        setApiFeaturedProducts(productsData.data);
        
        // Skip image preloading for faster initial load - do it in background
        console.log('📸 Image preloading skipped for faster loading');
      } else {
  ...
  // Search functionality - now works offline!
  const handleSearch = async (query) => {
    ...
    try {
      // Use hybrid API for offline search
      const result = await hybridStoreApi.searchProducts(query);
      console.log(`🔍 Search results from ${result.source}:`, result.data.length);
...
export default StoreScreen;    marginBottom: 10,
    borderRadius: 60,
  },
  categoryText: {
    fontSize: 14,
...
const StoreScreen = () => {
  ...
  const loadStoreData = async () => {
    try {
      ...
      // Use hybrid API that works offline
      const [health, categoriesResult, productsResult] = await Promise.allSettled([
        healthCheck().catch(err => {
          console.warn('⚠️ Health check error:', err);
          return { status: 'OFFLINE', message: err.message };
        }),
        hybridStoreApi.getCategories().catch(err => {
          console.warn('⚠️ Categories error:', err);
          return { source: 'offline', data: [] };
        }),
        hybridStoreApi.getProducts({ limit: 500 }).catch(err => {
          console.warn('⚠️ Products error:', err);
          return { source: 'offline', data: [] };
        })
      ]);
      ...
      if (productsData && productsData.data) {
        ...
        // Aggressively preload ALL product images when online
        if (networkStatus.isOnline && productsData.data.length > 0) {
          console.log('📸 Starting aggressive image caching for ALL products...');
          imageCacheService.preloadProductImages(productsData.data, 500)
            .then(() => console.log('✅ ALL product images queued for caching'))
            .catch(err => console.warn('⚠️ Image preload error:', err.message));
        }
      } else {
        console.warn('⚠️ Products loading failed, using fallback');
        setApiFeaturedProducts([]);
