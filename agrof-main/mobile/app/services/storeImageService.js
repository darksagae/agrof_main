...
class StoreImageService {
  ...
  getProductImage(product) {
    ...
    // NO FALLBACK - Return null if no real image is available
    // This prevents showing wrong generic category images for actual products
    console.log('⚠️ No real image available for product:', product.name);
    return null;
  }
  ...
  getOptimizedImageSource(product) {
    ...
    // Return null if no image available (don't show wrong generic images)
    if (!imageSource) {
  ...
  getImageSources(product) {
    ...
    // NOTE: Local bundled images in assets/store/ cannot be loaded dynamically
    // due to Metro bundler limitations (requires static require() paths).
    // For offline functionality, we rely on React Native's built-in image caching.
    // Images loaded once when online will be cached and available offline.

    // PRIORITY 2: Direct HTTP URL if available
    if (product.image_url && product.image_url.startsWith('http')) {
      console.log('✅ Using direct HTTP URL:', product.image_url);
      sources.push({ uri: product.image_url });
    }

    // PRIORITY 3: Backend API image URL (works when online)
    ...
    // PRIORITY 4: Full image URL if available
    ...
    // PRIORITY 5: Constructed store image URLs with different extensions (last resort)
