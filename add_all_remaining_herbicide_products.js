// Generate all remaining herbicide products (11-54)
const fs = require('fs');
...
// Generate products 11-54 (skip first 10 since they already exist)
console.log('// Products 11-54 to be added:');
for (let i = 10; i < files.length; i++) {
  const file = files[i];
  const nameWithoutExt = file.replace(/\.[^/.]+$/, "");
  const productName = productNames[i] || `Herbicide Product ${i + 1}`;
...
console.log('');
console.log(`// Total products to add: ${files.length - 10}`);
