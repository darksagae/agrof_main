...
echo "📋 This script will deploy both backends using Firebase:"
echo "   1. Node.js Store Backend → Firebase Functions"
echo "   2. Python Flask API → Firebase Functions (via Node.js wrapper)"
echo ""
...
echo "🔧 Step 1: Initialize Firebase Functions..."

# Initialize Firebase Functions
npx firebase-tools@latest init functions

echo "✅ Firebase Functions initialized"

echo "🔧 Step 2: Configure Firebase Functions for Store Backend..."

# Create functions package.json
cat > functions/package.json << 'EOF'
{
  "name": "agrof-functions",
  "description": "AGROF Backend Functions",
  "scripts": {
...
echo "🔧 Step 3: Create Firebase Functions for Store Backend..."
...
// Database setup (using Firebase Firestore instead of SQLite)
const db = admin.firestore();

// Product translation service
const loadProductTranslations = async (language = 'en') => {
  try {
    // Load from Firestore or fallback to static files
...
// Export individual functions for better organization
...
exports.categories = functions.https.onRequest(async (req, res) => {
  // Categories endpoint logic here
  res.json({ message: 'Categories endpoint' });
});

exports.products = functions.https.onRequest(async (req, res) => {
  // Products endpoint logic here
  res.json({ message: 'Products endpoint' });
});
EOF

echo "✅ Created Firebase Functions for Store Backend"

echo "🔧 Step 4: Create Python AI API as Firebase Function..."

# Create Python AI API function
cat > functions/ai-api.js << 'EOF'
const functions = require('firebase-functions');
...
exports.analyzeDisease = functions.https.onRequest(async (req, res) => {
  ...
  try {
    ...
    // In production, you would integrate with your Python AI service
    const mockAnalysis = {
      status: 'success',
...
echo "✅ Created Firebase Functions for AI API"

echo "🔧 Step 5: Update Firebase configuration..."

# Update firebase.json to include functions
cat > firebase.json << 'EOF'
{
  "hosting": {
    "public": "agrof-main/mobile/app/dist",
    "ignore": [
      "firebase.json",
      "**/.*",
      "**/node_modules/**"
...
echo "✅ Updated firebase.json with Functions configuration"

echo "🔧 Step 6: Create deployment script..."

cat > deploy-firebase-functions.sh << 'EOF'
#!/bin/bash
...
echo "   - Health Check: https://us-central1-agrof-ai-app.cloudfunctions.net/health"
echo ""
echo "🔗 Update your frontend to use these URLs!"
EOF

chmod +x deploy-firebase-functions.sh
...
echo ""
echo "📋 Next steps:"
echo "   1. Run: ./deploy-firebase-functions.sh"
echo "   2. Update your frontend API URLs"
echo "   3. Test the complete system"
echo ""
echo "🌐 Expected Firebase Functions URLs:"
echo "   - Store API: https://us-central1-agrof-ai-app.cloudfunctions.net/api"
echo "   - AI API: https://us-central1-agrof-ai-app.cloudfunctions.net/analyzeDisease"
echo "   - Health: https://us-central1-agrof-ai-app.cloudfunctions.net/health"
echo ""
echo "💡 Firebase Functions provide:"
echo "   - Serverless backend"
echo "   - Automatic scaling"
echo "   - Pay-per-use pricing"
echo "   - Integrated with your existing Firebase project"
